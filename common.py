...
def save(data):
    """存储数据"""
    engine = create_engine('mysql+pymysql://{}:{}@{}:{}/{}?charset=utf8'.format(USER, PASS, HOST, PORT, DB),pool_recycle=10, pool_size=200)
    connect = engine.connect()
    try:
        df = pd.DataFrame(data, index=[0])
        df.to_sql(name=TABLE, con=connect, if_exists='append', index=False, chunksize=100)
